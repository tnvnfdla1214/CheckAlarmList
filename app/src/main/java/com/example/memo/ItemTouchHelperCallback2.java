...
public class ItemTouchHelperCallback2 extends ItemTouchHelper.Callback  {
    ...
    private ButtonsState2 buttonsShowedState = ButtonsState2.GONE;
    private static final float buttonWidth = 115;
    private RectF buttonInstance = null;
    private RecyclerView.ViewHolder currenrtItemViewHolder = null;
    public ItemTouchHelperCallback2(ItemTouchHelperListener listener) {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (buttonsShowedState != ButtonsState2.GONE) {
                if (buttonsShowedState == ButtonsState2.LEFT_VISIBLE) dX = Math.max(dX, buttonWidth);
                if (buttonsShowedState == ButtonsState2.RIGHT_VISIBLE)
    ...
    private void drawButtons(Canvas c, RecyclerView.ViewHolder viewHolder){     //버튼을 그려주는 함수
        ...
        buttonInstance = null;
        ////왼쪽 버튼
        if(buttonsShowedState == ButtonsState2.LEFT_VISIBLE){
        ...
        ////오른쪽 버튼
        else if(buttonsShowedState == ButtonsState2.RIGHT_VISIBLE){
    ...
    private void setTouchListener(final Canvas c, final RecyclerView recyclerView, final RecyclerView.ViewHolder viewHolder, final float dX, final float dY, final int actionState, final boolean isCurrentlyActive){
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(swipeBack){
                    if(dX < -buttonWidth) {
                        buttonsShowedState = ButtonsState2.RIGHT_VISIBLE;
                    }
                    else if(dX > buttonWidth) {
                        buttonsShowedState = ButtonsState2.LEFT_VISIBLE;
                    }
                    if(buttonsShowedState != ButtonsState2.GONE){
    ...
    private void setTouchUpListener(final Canvas c, final RecyclerView recyclerView , final RecyclerView.ViewHolder viewHolder, final float dX, final float dY , final int actionState, final boolean isCurrentlyActive){
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                ItemTouchHelperCallback2.super.onChildDraw(c, recyclerView, viewHolder, 0F, dY, actionState, isCurrentlyActive);
                recyclerView.setOnTouchListener(new View.OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {
                        return false;
                ...
                buttonsShowedState = ButtonsState2.GONE;
